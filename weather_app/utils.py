...
def get_xth_hour_weather(hour, weather_data):
  """
  Returns the xth hour of the forecast. 
  Weather data includes temperature, humidity, wind, precipitation.
  """
  if len(weather_data["temperature"]) < hour:
    raise ValueError("Hour index is too large")
  
  return [
    int(weather_data[x][hour]) 
    for x in ['temperature', 'humidity', 'wind', 'precipitation']
  ]
